...
void do_homing_move(const AxisEnum axis, const float distance, const feedRate_t fr_mm_s) {
  ...
  if (is_home_dir) {
    ...
    planner.buffer_segment(target, fr_mm_s*60.0f, active_extruder);
  #else
    abce_pos_t target = { planner.get_axis_position_mm(A_AXIS), planner.get_axis_position_mm(B_AXIS), planner.get_axis_position_mm(C_AXIS), planner.get_axis_position_mm(E_AXIS) };
    target[axis] = 0;
    planner.set_machine_position_mm(target);
...
void homeaxis(const AxisEnum axis) {
  ...
  // if (DEBUGGING(LEVELING)) 
  DEBUG_ECHOLNPGM("Home 1 Fast:");

  #if HOMING_Z_WITH_PROBE && ENABLED(BLTOUCH)
    if (axis == Z_AXIS && bltouch.deploy()) return; // The initial DEPLOY
  #endif

  do_homing_move(axis, 1.5f * axis_home_dir * max_length(axis),homing_feedrate(axis));
  ...
  if (bump) {
    ...
    // if (DEBUGGING(LEVELING)) 
    DEBUG_ECHOLNPGM("Move Away:");
    do_homing_move(axis, -bump, get_homing_bump_feedrate(axis));

    // Slow move towards endstop until triggered
    // if (DEBUGGING(LEVELING)) 
    DEBUG_ECHOLNPGM("Home 2 Slow:");
